...
int n, k;
vvi adj;
vvi dp;  // dp[i][k]= sum of all dp[it][k-1]where it is a child node. TC = n * k <=1e7
int answer = 0;
void dfs(int node, int par) {
    for (auto &it : adj[node]) {
        if (it != par) {
            ...
            answer += dp[it][k - 1];
            for (int j = 0; j <= k - 2; j++) {
                answer += 1ll * dp[it][j] * dp[node][k - j - 1];
            }
            for (int j = 1; j <= k; j++) {
                dp[node][j] += dp[it][j - 1];
            }
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        dp[i][0] = 1;  // number of paths such that shortest distance = 0 will be 1 (that node itself)
    }
    dfs(1, -1);
    cout << answer << nl;
}
