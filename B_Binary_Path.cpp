...
void solve() {
    ...
    ll row = 0;
    for (ll i = 0; i < n; i++) {
        // cout << "at " << arr[row][i] << nl << row << " " << i << nl;
        if (i != n - 1 || row != 1) {
            // cout << "adding " << row << " : " << i << nl;
            s += arr[row][i];
        }
        if (row == 0) {
            if (i + 1 < n)
                if (arr[0][i + 1] == '1' && arr[1][i] == '0') {
                    // cout << "switching at " << i << nl;
                    s += '0';
                    row = 1;
                }
        }
    ...
    // cout << firstIndex << " " << secondIndex << nl;
    ll counter = firstIndex - secondIndex + 1;
    cout << s << nl << counter << nl;
}
int main() {
