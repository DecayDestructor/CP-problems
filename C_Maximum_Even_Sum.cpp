...
void solve() {
    ...
    int answer = a + b;
    // cout << "answer ";
    if (two[1] == 0 && two[0] >= 1) {  // b is odd, a is even
        cout << -1 << nl;
        return;
    } else if (two[0] == 0 && two[1] == 1) {  // b has only one factor of 2, while a is odd
        cout << -1 << nl;
        return;
    } else if (two[0] != 0 && two[1] != 0) {  // a and b both are even
        int rem = tempb / 2;
        tempa = 1ll * tempa * rem;
        cout << tempa + 2 << nl;
        return;
    } else if (two[0] == two[1] && two[0] == 0)  // a and b are both odd
    {
        // cout << tempa << " " << tempb << nl;
        cout << 1ll * a * b + 1 << nl;
        return;
    } else {  // valid even odd case (a=odd, b=even)
        int rem = tempb / 2;
        tempa = 1ll * tempa * rem;
        cout << tempa + 2 << nl;
        return;
