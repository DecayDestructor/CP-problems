...
void solve() {
    string str, pattern;
    cin >> str >> pattern;
    Hashing hash1(pattern);
    Hashing hash2(str);
    int requiredHash1 = hash1.substringHash(0, pattern.length() - 1)[0];
    int requiredHash2 = hash1.substringHash(0, pattern.length() - 1)[1];
    int l = 0, r = pattern.length() - 1;
    int counter = 0;
    while (r < str.length()) {
        int foundHash1 = hash2.substringHash(l, r)[0];
        int foundHash2 = hash2.substringHash(l, r)[1];
        if ((foundHash1 == requiredHash1) && (foundHash2 == requiredHash2)) counter++;
        l++, r++;
    }
    cout << counter << nl;
}
