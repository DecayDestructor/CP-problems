...
void solve() {
    ...
    ll index = n - 1;
    while (index > m - 1) {
        while (index > m - 1 && b[index] <= a[index]) {
            coins += b[index];
            index--;
        }
        while (index > m - 1 && a[index] < b[index]) {
            coins += a[index];
            index--;
        }
    }
    ...
    for (ll i = 1; i <= m - 1; i++) {
        ...
        // cout << "min(" << a[i] << "," << b[i] << "+" << dp[i - 1] << ")" << nl;
        // cout << "value at index " << i << " is " << dp[i] << nl;
    }
    cout << dp[m - 1] + coins << nl;
}
int main() {
