...
void solve() {
    ...
    while (right <= n) {
        // cout << left << " : " << right << nl;
        while (right <= n && getSum(left, right) < v) {
            if (right - 1 >= 0) {
                prefix[right] = prefix[right - 1];
            }
            right++;
        }
        while (left <= n && left <= right && getSum(left, right) >= v) {
            cout << left << "->" << right << nl;
            cout << prefix[right] << " : " << prefix[left] << endl;
            prefix[right] = prefix[left] + 1;
            // cout << right << " : " << prefix[right] << nl;
            left++;
        }
    }
    ...
    while (right <= n) {
        ...
        while (left < n && left <= right && getSum(left, right) >= v) {
            suffix[right] = suffix[left] + 1;
            left++;
        }
        // if(right==n && getSum(left,right)<v)
    }
