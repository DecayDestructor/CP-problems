...
void dfs(int node, vector<vi> &adj, vi &visited, int &connectable, vi &indigree) {
    if (visited[node]) return;
    visited[node] = 1;
    if (indigree[node] < 2) connectable++;
    for (auto &it : adj[node]) {
        if (!visited[it])
            dfs(it, adj, visited, connectable, indigree);
    }
    return;
}
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        if (!visited[i]) {
            count++;
            int connectable = 0;
            dfs(i, adj, visited, connectable, indigree);
            ind_count[connectable]++;
        }
    }
    cout << ind_count[0] + min(ind_count[2], 1ll) << " " << count << nl;
}
signed main() {
