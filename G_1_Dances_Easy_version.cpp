...
ll check(vll &b, ll num, vll &visited) {
    ll n = b.size();
    ll left = 0, right = n - 1;
    ll ans = (ll)1e6;
    while (left <= right) {
        ll middle = (left + right) >> 1;
        if (b[middle] > num && visited[middle] != 1) {
            ans = min(middle, ans);
            right = middle - 1;
        } else {
            left = middle + 1;
...
void solve() {
    ...
    for (ll i = 0; i < n; i++) {
        // ll left = 0, right = n - 1;
        // cout << "checking for " << a[i] << nl;
        ll index = check(b, a[i], visited);
        if (index != -1) {
            // cout << "found the smallest number greater than the current one at index " << index << nl;
            visited[index]++;
            counter++;
        }
    }
