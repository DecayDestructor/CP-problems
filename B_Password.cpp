...
void solve() {
    ...
    Hashing hash(s);
    cout << hash.substringHash(len - 3, len - 1)[0] << " : " << hash.substringHash(0, 2)[1] << nl;
    while (left <= right) {
        int middle = left + (right - left) / 2;
        bool ok = false;
        cout << middle << " : " << left << " : " << right << nl;
        if (middle == 2) cout << "in the 2 " << nl;
        for (int i = middle; i + middle - 1 < len; i++) {
            if (middle == 2) {
                cout << hash.substringHash(0, middle - 1)[0] << " , " << hash.substringHash(0, middle - 1)[1] << " : " << hash.substringHash(middle, middle + middle - 1)[0] << " , " << hash.substringHash(middle, middle + middle - 1)[1] << nl;
            }

            if (hash.substringHash(0, middle - 1) == hash.substringHash(middle, middle + middle - 1)) ok = true;
        }
        if (ok) {
            left = middle + 1;
            prefix = 0;
