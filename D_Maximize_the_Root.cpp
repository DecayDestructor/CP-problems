...
int dfs(int node, vector<vi> &adj, vi &arr) {
    // if (visited[node]) return arr[node];
    if (adj[node].size() == 0) {
        return arr[node];
    }
    int val = arr[node];
...
void solve() {
    ...
    int finalNode = 1e9;
    for (auto &it : adj[1]) {
        finalNode = min(finalNode, dfs(it, adj, arr));
    }
    cout << finalNode + arr[1] << nl;
}
signed main() {
