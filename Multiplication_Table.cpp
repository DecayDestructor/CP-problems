...
bool helper(int n, int middle) {
    int tempN = n;
    int counter = 0;
    // cout << "for " << middle << nl;
    while (tempN) {
        // cout << tempN << " : " << max((n - ((middle + tempN - 1) / tempN)) + 1, 0ll) << nl;
        counter += max((n - ((middle + tempN - 1) / tempN)) + 1, 0ll);
        tempN--;
    }
    // cout << middle << " : " << counter << nl;
    return counter - 1 >= (1ll * n * n) / 2;
}
void solve() {
    int n;
    cin >> n;
    int left = 1, right = 1ll * n * n;
    int answer = 1;
    while (left <= right) {
