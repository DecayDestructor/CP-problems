...
void solve() {
    ...
    // priority_queue<pair<ll, ll>, vector<pair<ll, ll>>, decltype(cmp)> powerups(cmp);
    priority_queue<ll, vll> powerups;
    for (ll i = 0; i < n; i++) {
        ll a, b;
        cin >> a >> b;
        hurdles[a] = b - a + 2;
    ...
    // }
    // cout << p[2].first << " " << p[2].second << nl;
    for (auto &it : hurdles) {
        ll x = it.first;
        ll req = it.second;
        while (index < m && p[index].first <= x) {
            // cout << "pushing " << p[index].first << " " << p[index].second << nl;
            powerups.push(p[index].second);
            index++;
        }
        // cout << "after pushing index is " << p[index].first << nl;
        while (curr < req && powerups.size() != 0) {
            curr += powerups.top();
            // cout << "value of curr at " << x << "  is " << curr << nl;
            answer++;
            powerups.pop();
        }
        if (curr < req) {
            // cout << req << " is the requried powerup and I had " << curr << nl;
            cout << -1 << nl;
            return;
        }
    }
