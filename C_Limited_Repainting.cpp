...
ll little_fermat(ll a, ll m) {
    return binpow(a, m - 2, m);
}
bool helper(int x, int n, int k, string &s, vector<int> &arr) {
    int counter = 0, i = 0;
    while (i < n) {
        if (s[i] == 'R' && arr[i] > x) {
            i++;
        ...
        // cout << "at index " << i << nl;
        bool flag = false;
        while (i < n && !(s[i] == 'R' && arr[i] > x)) {
            if (s[i] == 'B' && arr[i] > x) {
                // cout << s[i] << " pen is more " << nl;
                flag = true;
            }
            i++;
        }
...
void solve() {
    ...
    int left = 0, right = (int)maxi, ans = (int)maxi;
    while (left <= right) {
        // cout << "checking for " << middle << nl;
        int middle = left + (right - left) / 2;
        if (helper(middle, n, k, s, arr)) {
            // cout << "curr answer is " << answer << nl;
            ans = middle;
            right = middle - 1;
        } else {
            left = middle + 1;
