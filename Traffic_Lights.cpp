...
void solve() {
    ...
    set<int, decltype(cmp)> lights(cmp);
    ...
    set<pi, decltype(dist)> intervals(dist);
    lights.insert(0);
    lights.insert(n);
    intervals.insert({0, n});
    while (q--) {
        int x;
        cin >> x;
        int pre = *prev(lights.lower_bound(x));
        int nex = *lights.upper_bound(x);
        intervals.erase({pre, nex});
        intervals.insert({pre, x});
        intervals.insert({x, nex});
        lights.insert(x);
        pi beg = *intervals.begin();
        cout << beg.second - beg.first << " ";
    }
    cout << nl;
}
signed main() {
