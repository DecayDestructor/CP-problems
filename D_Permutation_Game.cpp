...
long long helper(vector<ll> &p, vector<ll> &a, int s, int k) {
    int n = p.size();
    long long mx = 0, cur = 0;
    vector<bool> vis(n);
    while (!vis[s - 1] && k > 0) {
        vis[s - 1] = 1;
        mx = max(mx, cur + 1ll * k * a[s - 1]);
        cur += a[s - 1];
        k--;
        s = p[s - 1];
    }
...
void solve() {
    ...
    if (helper(a, b, pb, k) > helper(a, b, ps, k))
        cout << "Bodya" << nl;
    else if (helper(a, b, pb, k) < helper(a, b, ps, k))
        cout << "Sasha" << nl;
    else
        cout << "Draw" << nl;
    // cout << helper(a, b, pb, k) << nl;
}
