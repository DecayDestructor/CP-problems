...
void solve() {
    ...
    for (auto &it : mapb) {
        if (it.second > mapd[it.first]) {
            // cout << "value of " << it.first << " is less in the d array" << nl;
            pn;
            return;
        }
    }
    ll last = *d.rbegin();
    if (!elements.count(last)) {
        // cout << "last element is not present in b array as the last element was " << last << nl;
        // cout << " the elements of the map are " << nl;
        // for (auto it : mapb)
