...
bool helper(int middle, vll &points, vll &towers) {
    for (auto &it : points) {
        int lower = it - middle, higher = it + middle;
        auto bound = lower_bound(all(towers), lower);
        if (bound == towers.end() || *bound > higher) {
            return false;
        }
    }
    return true;
...
void solve() {
    ...
    while (left <= right) {
        ...
        if (helper(middle, points, towers)) {
