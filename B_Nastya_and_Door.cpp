...
void solve() {
    ...
    ll right = k - 1;
    ll ans = prefix[right];
    if (prefix[right - 1] != prefix[right]) {
        ans--;
    }
    ll index = left;
    // debug(prefix);
    // cout << "peak from " << left << " to " << right << " is " << ans << nl;
    while (right + 1 < n) {
        right++;
        left++;
        ll peaks = prefix[right] - prefix[left];
        // if (prefix[left - 1] != prefix[left]) {
        //     peaks++;
        // }
        if (prefix[right - 1] != prefix[right]) {
            peaks--;
        }
        // cout << "peak from " << left << " and " << right << "  is " << peaks << nl;
        if (peaks > ans) {
            index = left;
            ans = peaks;
            // cout << "updated peaks to " << peaks << " and updated value to " << index << nl;
        }
    }

    cout << ans + 1 << " " << index + 1 << nl;
}
int main() {
