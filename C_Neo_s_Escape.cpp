...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        temp.for (int j = i + 1; j < n && arr[i].first == arr[j].first; j++);
    }
    ...
    for (int i = 0; i < n; i++) {
        auto it = arr[i];
        if (it.second - 1 >= 0 && dp[it.second - 1]) {
            dp[it.second] = true;
        } else if (it.second + 1 < n && dp[it.second + 1]) {
            dp[it.second] = true;
        } else {
            answer++;
            dp[it.second] = true;
        ...
        if (i + 1 < n) {
            ...
            int pos = arr[i + 1].second;
            if (it.second + 1 < n && temp[it.second + 1].first == next) {
                dp[it.second + 1] = true;
            }
            if (it.second - 1 >= 0 && temp[it.second - 1].first == next) {
                dp[it.second - 1] = true;
            }
        }
    }
