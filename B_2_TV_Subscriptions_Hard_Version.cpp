...
void solve() {
    ...
    // cout << ans << nl;
    int left = d;
    // cout << "Starting at index " << d << nl;
    while (left < n) {
        // cout << "answer is now " << ans << nl;
        // for (auto &it : mpp)
        //     cout << it.first << " " << it.second << nl;

        // cout << "erasing " << arr[left - d] << nl;
        if (left - d >= 0)
            mpp[arr[left - d]]--;
        if (mpp[arr[left - d]] == 0) {
            // cout << arr[left - d] << " deleted " << nl;
            mpp.erase(arr[left - d]);
            // cout << "new size is " << mpp.size() << nl;
        }

        // if (left < n)
        //     cout << "adding " << arr[left] << nl;
        if (left < n)
            mpp[arr[left++]]++;
        ans = min(ans, (ll)mpp.size());
    }
