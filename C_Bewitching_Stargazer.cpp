...
ll helper(ll l, ll r, ll &k) {
    if (r - l + 1 < k || l == r) {
        // cout << l << " to " << r << " is less than " << k << nl;
        return 0;
    }
    ll middle = (l + r) / 2;
    // cout << "middle of " << l << " " << r << "  is " << middle << nl;
    isEven(r - l + 1) {
        // cout << l << " : " << r << " is even length and " << middle << " is not added " << nl;
        return helper(l, middle, k) + helper(middle + 1, r, k);
    }
    else {
        // if (l != r)
        // cout << l << " : " << r << " is odd  length and " << middle << " is added " << nl;
        return middle + helper(l, middle - 1, k) + helper(middle + 1, r, k);
    }
}
void solve() {
    ll n, k;
    cin >> n >> k;
    cout << helper(1, n, k) << nl;
}
int main() {
