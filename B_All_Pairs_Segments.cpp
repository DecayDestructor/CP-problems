...
void solve() {
    ...
    for (ll i = 0; i < n; i++) {
        ll val = (n - 1 - i) + (n - 1 - i + 1) * i;
        // cout << "segment " << val << " incremented due to " << arr[i] << " at position i" << nl;
        mpp[val]++;
        if (i - 1 >= 0) {
            mpp[(n - 1 - i + 1) * i] += (arr[i] - arr[i - 1] - 1);
            // cout << "segment " << (n - i + 1) * i << " incremented by " << arr[i] - arr[i - 1] - 1 << " due to " << arr[i] << " and " << arr[i - 1] << nl;
        }
    }
    for (auto &it : queries) {
        cout << mpp[it] << " ";
