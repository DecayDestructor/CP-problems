...
int gain(int t, int u) {
    return (u + (u - t + 1)) * t / 2;
}
int checker(int sum, int &u) {
    if (sum <= u) {
        return sumOfNaturalNumbers(u) - sumOfNaturalNumbers(u - sum);
    } else {
        return sumOfNaturalNumbers(u) - sumOfNaturalNumbers(sum - (u + 1));
    }
}
void solve() {
    int n;
    ...
    while (q--) {
        ...
        while (left <= right) {
            ...
            if (prefix[middle] - prefix[l - 1] <= u) {
                index = max(middle, index);
                left = middle + 1;
            } else
                right = middle - 1;
        }
        int finalindex = index;
        int currmax = checker(prefix[index] - prefix[l - 1], u);
        for (int i = 1; i <= 2; i++) {
            if (index + i <= n) {
                int check = checker(prefix[index + i] - prefix[l - 1], u);
                if (check > currmax) {
                    currmax = check;
                    finalindex = index + i;
                }
