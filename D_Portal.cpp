...
void solve() {
    ...
    x = tempx, y = tempy;
    rotate(arr.begin() + tempx,
           arr.begin() + index,
           arr.begin() + tempy);
    vi ans;
    vi inside;
    while (x < y) inside.push_back(arr[x++]);
    x = tempx;
    vi outside;
    for (int i = 0; i < n; i++) {
        if (i < x || i >= y) outside.push_back(arr[i]);
    }
    int i = 0, j = 0;
    while (i < outside.size()) {
        if (outside[i] <= inside[0]) {
            ans.push_back(outside[i]);
            i++;
        } else
            break;
    }
    while (j < inside.size()) ans.push_back(inside[j++]);
    while (i < outside.size()) {
        ans.push_back(outside[i++]);
    }
