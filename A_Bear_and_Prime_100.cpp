...
void sieveOfEratosthenes(int n, vll &ans) {
...
string ask(ll x) {
    cout << x << nl;
    if (testing) return interactor(x);
    string res;
    cin >> res;
    return res;
}
void solve() {
    vll arr;
    sieveOfEratosthenes(100, arr);
    // debug(arr);
    // cout << nl;
    ll queries = 0;
    for (int i = 0; queries < 20; i++) {
        string s = ask(arr[i]);
        queries++;
        if (s == "yes") {
            for (int j = 0; queries < 20; j++) {
                queries++;
                if (arr[j] * arr[i] > 100) {
                    cout << "prime" << endl;
                    return;
                }
                string res = ask(arr[i] * arr[j]);
                if (res == "yes") {
                    cout << "composite" << endl;
                    return;
                }
