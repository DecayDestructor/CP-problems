...
void backtrack(int index, long long currSum) {
    if (index == n) {
        ans = min(ans, abs(total - 2 * currSum));
        return;
    }

    // Option 1: include arr[index] in subset 1
    backtrack(index + 1, currSum + arr[index]);

    // Option 2: do not include arr[index] in subset 1 (go to subset 2)
