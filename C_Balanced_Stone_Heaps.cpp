...
bool check(ll &middle, vll &arr) {
    vll v = arr;
    // debug(arr);
    for (ll i = arr.size() - 1; i - 2 >= 0; i--) {
        ll transfer = (v[i] - middle) / 3;
        // cout << "transferring " << transfer << " from " << v[i] << nl;
        ll req = min(transfer, arr[i] / 3);
        if (req < 0) {
            return false;
        }
        v[i - 1] += req;
    ...
    // cout << nl;
    // cout << "for middle value " << middle << nl;
    // debug(v);
    for (auto it : v) {
        if (it < middle) {
            return false;
